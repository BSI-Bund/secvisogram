 ...
 *   onNewFormDoc(): void
 *   onNewSourceDoc(): Promise<void | {}>
 *   onStrip(document: {}): Promise<{ document: {}, strippedPaths: Array<{ dataPath: string; message: string; error: boolean }> }>
 * }} props
 */
function View({
  activeTab,
  ...
  const onUpdate = React.useCallback((/** @type {{}} */ newValue) => {
    dispatch({
  ...
  return (
    <div className="mx-auto w-full h-screen flex flex-col">
      ...
      <div className="relative overflow-auto h-full" key={activeTab}>
        {isLoading ? (
