...
export default class CVSSMetrics {
  /**
   ...
   */
  set(property, key) {
    return new CVSSMetrics({ ...this.data, [property]: key })
  }
  ...
  get vectorString() {
    const result = CVSS31.calculateCVSSFromMetrics(
       ...
       *   string,
       * ]}
       */ (vectorStringMapping.map((e) => e[2][this.data[e[0]]]))
    )
    return result.success ? result.vectorString : ''
  }
  ...
  updateFromVectorString(vectorString) {
    ...
    return new CVSSMetrics({ ...this.data, ...data })
  }
}
