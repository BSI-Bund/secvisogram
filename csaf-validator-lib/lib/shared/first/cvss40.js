...
class Vector {
  ...
  get severityBreakdown() {
    ...
    const macroVectorsWithTwoSeverities = [
      'Complexity',
      'Security requirements',
    ]
...
export class CVSS40 {
  ...
  calculateScore() {
    ...
    // EQ values
    const [eq1, eq2, eq3, eq4, eq5, eq6] = equivalentClasses
      .split('')
      .map(Number)
    ...
    if (eq3 == 0 && eq6 == 0) {
      ...
      const score_eq3eq6_next_lower_macro_left =
        CVSS40.LOOKUP_TABLE[eq3eq6_next_lower_macro_left]
      ...
      score_eq3eq6_next_lower_macro =
        CVSS40.LOOKUP_TABLE[eq3eq6_next_lower_macro]
    }
